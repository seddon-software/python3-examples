...
class Degas:
    def __init__(self, motor, motorMax, gauge, frontend, ringCurrent, 
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! cahnging next line for debugging
#        self.monitorFreq = 0.1
    ...
    def updatePosition(self, pressure):
        ...
        if (abs(error) < self.pressureDeadband):
            self.printMessage("pressure in deadband: {:.3e}, not moving device".format(pressure))
            return
        ...
        if (newPosition == currentPosition):
            self.printMessage("pressure {:.3e} , not moving device (at max position)".format(pressure))
        else:
            self.printMessage("pressure {:.3e} , moving device to {:.3g}".format(pressure, newPosition))
...
# minPressure = 5e-9, targetPressure = 3e-8, pressureDeadband = 0.2e-8, maxPressure = 5e-8,
# maxForwardMovement = 0.05):
    def __init__(self, initialPressure):
        self.pressure = initialPressure
        self.count = 4
...
class Motor:
    def __init__(self, startPosition, maxPosition, gauge):
    ...
    def moveTo(self, newPosition):
        change = self.position - newPosition
        self.gauge.motorChange(change)
        self.position = newPosition

    def __call__(self):
        while(True):
            sleep(dt * 100)
            gauge.releasePressure()
        
    def asynchronousMoveTo(self, position):
        self.position = position
    
gauge = Gauge(2e-8)
motor = Motor(2.0, 3.0, gauge)
frontend = FrontEnd()
ringCurrent = RingCurrent()

d = Degas(motor, motor.getMaxPosition(), gauge, frontend, ringCurrent)
d.updatePosition(0)
d.run()
