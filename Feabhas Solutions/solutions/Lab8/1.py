...
addr_list = []
addr_input = ""
while (addr_input != "quit"):
    addr_input = raw_input("Enter an IP Address: ")
    # Make the string lower case for the quit check
    addr_input =addr_input.lower()
    if (addr_input != "quit"):
        addr_list.append(addr_input)
...
for addr in addr_list:
    ...
    if (len(dotted_quad) != 4):
        ...
        print "Error with the address {1}: {0}".format(message, dotted_quad)
